...
**Status: CPU relay tests and simulated Slurm handoff pass. No real-weight GPU
inference, recurrent-state correctness, CUDA graph, or performance validation
has been completed.** Use an experimental job before relying on its outputs.

The first supported experiment is target TP=1/PP=2 or 4 with DFlash draft
TP=1/PP=1. The launcher requires eager execution, an unextended context, and no
HF overrides. It defaults eager execution on when the opt-in is enabled.
The patch rejects other methods, target TP>1, sequence parallelism, and other
model types. This does not add DSpark/EAGLE3 presets or change any quantization.
...
exact already-patched source. It refuses locally modified files and different
versions. Applying twice is harmless; writes use a lock and atomic replacement.
...
unset SPEC_MODEL SPEC_TOKENS HF_OVERRIDES
export TACC_QWEN3NEXT_PP_DFLASH=1
export VLLM_USE_V2_MODEL_RUNNER=1
export TP_SIZE=1 PP_SIZE=4 SPEC_TP_SIZE=1 SPEC_METHOD=dflash
export SPEC_TOKENS=15
...
correctness testing. Use `PP_SIZE=2` with `sbatch --nodes=2` for the first
two-node test. The four-node defaults remain TP=2/PP=2 with speculation off when
...
They compare 1,200 partition/tap combinations against the unpartitioned
...
opt-out behavior, scope guards, version refusal, and apply/check/revert.
They do not execute torch kernels, NCCL, tensor buffer allocation, or real
recurrent caches. The handoff test stubs vLLM imports/configuration in its
...
2. Compare real auxiliary tensors at PP=2/4 against the PP=1 reference: values
   within numerical tolerance, tap order, shapes, dtype, and token alignment.
